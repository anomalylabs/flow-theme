...
<div class="min-h-screen bg-white flex">
    ...
    <div class="mx-auto w-full max-w-sm">
        <div>
            {!! img('anomaly.theme.flow::img/logo.svg')->class('h-12 w-auto')->inline() !!}
            ...
            <div>
                ...
                @if (config('anomaly.module.users::login', 'email') == 'username')
                    {!! $form->fields->username
                        ->setAttribute('placeholder', $form->fields->username->label)
                        ->setAttribute('attributes.class', 'appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md placeholder-gray-400 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 transition duration-150 ease-in-out sm:text-sm sm:leading-5')
                        ->setAttribute('autofocus', 1)
                        ->render() !!}
                @else
                    {!! $form->fields->email
                        ->setAttribute('placeholder', $form->fields->email->label)
                        ->setAttribute('attributes.class', 'appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md placeholder-gray-400 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 transition duration-150 ease-in-out sm:text-sm sm:leading-5')
                        ->setAttribute('autofocus', 1)
                        ->render() !!}
                @endif
            ...
            <div class="mt-6">
                ...
                <div class="mt-1 rounded-md shadow-sm">
                    {!! $form->fields->password
                        ->setAttribute('placeholder', $form->fields->password->label)
                        ->setAttribute('attributes.class', 'appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md placeholder-gray-400 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 transition duration-150 ease-in-out sm:text-sm sm:leading-5')
                        ->render() !!}
                </div>
            </div>
